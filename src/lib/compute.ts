...
/**
 * 一站式派生所有排盘数据：八字基础派生 + 十神派生 + 强弱分析 + 格局检测 + 喜用分析
 * 用于store层一键更新所有结果
 */
export function deriveAll(r: BaziResult, gejuExtras: { dayun?: Pillar; liunian?: Pillar } = {}) {
  const baziDerived = deriveBazi(r)
  const shishenDerived = deriveShishen(r)
  const strengthDerived = deriveStrength(r.pillars)
  const gejuHits = detectGejuWith(baziDerived, shishenDerived, strengthDerived, gejuExtras)
  const xiyongAnalysis = analyzeXiyong(r.pillars, strengthDerived.analysis, gejuHits)
  return { ...baziDerived, ...shishenDerived, ...strengthDerived, gejuHits, xiyongAnalysis }
}
